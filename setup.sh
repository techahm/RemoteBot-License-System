...
read -p "$(echo -e ${Y}'🔗 licenses.json RAW URL     : '${X})" LICENSE_URL
read -p "$(echo -e ${Y}'🔑 public_key.pem RAW URL    : '${X})" PUBKEY_URL
...
pkg install -y python openssl curl termux-tools
...
# ═══════════════════════════════════════════════════════
# STEP 4: public_key.pem ডাউনলোড (Admin এর আসল key)
# ⚠️  এখানে কোনো নতুন key তৈরি হবে না!
# ═══════════════════════════════════════════════════════
echo -e "${Y}${B}📋 STEP 4: Public Key ডাউনলোড হচ্ছে...${X}"
echo -e "${C}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${X}"

# পুরনো ভুল key মুছে দাও
rm -f public_key.pem private_key.pem

# Admin এর আসল public_key.pem ডাউনলোড করো
curl -sL "${PUBKEY_URL}" -o public_key.pem

# চেক করো
if [ -f "public_key.pem" ] && grep -q "PUBLIC KEY" public_key.pem 2>/dev/null; then
    echo -e "${G}✅ public_key.pem সফলভাবে ডাউনলোড হয়েছে${X}"
else
    echo -e "${R}❌ public_key.pem ডাউনলোড ব্যর্থ!${X}"
    echo -e "${Y}👉 Admin এর কাছ থেকে সঠিক public_key.pem URL নিন।${X}"
    exit 1
fi
echo ""
...
if [ ! -f "public_key.pem" ]; then
    echo -e "${R}❌ public_key.pem নেই! আবার setup.sh রান করুন।${X}"; exit 1
fi
if ! grep -q "PUBLIC KEY" public_key.pem 2>/dev/null; then
    echo -e "${R}❌ public_key.pem ভুল! আবার setup.sh রান করুন।${X}"; exit 1
fi
...
if [ -n "$OLD_PID" ]; then
    echo -e "${Y}⚠️  আগের বট বন্ধ হচ্ছে (PID: $OLD_PID)...${X}"
...
echo -e "${C}📋 Log দেখতে : tail -f remotebot.log${X}"
echo -e "${C}🛑 বন্ধ করতে : bash stop.sh${X}"
echo ""
...
echo " ╠══════════════════════════════════════════════════╣"
echo " ║  ⚠️  Termux:Boot (F-Droid) ইন্সটল করুন           ║"
